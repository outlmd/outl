...
| Populate that index from the **op-log tree** instead of from disk (id-carrying nodes, no sidecar, cannot drop a block on a stale hash) | `outl_md::IdentifiedNode` + `WorkspaceIndex::insert_page` / `collect_page_blocks_from_tree` / `collect_refs_from_indexed` | `crates/outl-md/src/index.rs`, `block_index.rs` |
