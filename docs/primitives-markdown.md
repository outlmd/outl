...
| The same verdict from block **texts** alone, for the one caller whose reference is a render and not a sidecar. Byte-identical result; it exists so the survey stops minting a throwaway ULID + SHA-256 per block on every sweep | `outl_md::unlogged::content_lines_missing_from_texts` | `crates/outl-md/src/unlogged.rs` |
...
| Own a publish-by-rename scratch file so it is unlinked on **every** in-process exit — an early `?`, a panic, a cancelled future — instead of on the one failure path someone remembered | `outl_md::atomic::TempFile` (`new` / `path` / `keep`) | `crates/outl-md/src/atomic.rs` |
| `fsync` a parent directory so a `rename` into it is durable (best-effort; Windows refuses to open a directory) | `outl_md::atomic::sync_dir` | `crates/outl-md/src/atomic.rs` |
