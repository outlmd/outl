...
The concrete cost of keeping the exclusion was `outl-cli`'s `outl theme list`, which printed the TUI's own copy of the list. **Amended:** the TUI's `PRESETS` const was deleted and `crates/outl-tui/src/theme.rs` now re-exports `outl_theme::PRESETS`; `outl_tui::THEME_PRESETS` no longer exists and `outl theme list` reads `outl_theme::PRESETS` directly.
...
`outl-light` sits second in `outl_theme::PRESETS`, right after `outl`. **Amended:** there is no longer a separate TUI list to keep in the same position — `outl_tui::theme::PRESETS` is a re-export of this one.
