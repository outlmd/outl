...
| Abandoned `snap-*.bin.tmp.<ulid>` (and the shared `snap-*.bin.tmp` older builds left) past the TTL — a write that died mid-rename, not a cache the selector can reach | `outl_core::snapshot::gc::stale_tmp` / `prune_tmp` / `STALE_TMP_TTL` | `crates/outl-core/src/snapshot/gc.rs` |
