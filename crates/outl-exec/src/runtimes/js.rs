...
impl Runtime for JsRuntime {
    ...
    /// Runs on a worker thread because Boa 0.22 has no cancellation
    /// point: `RuntimeLimits` caps loop iterations, recursion depth and
    /// stack size, but nothing per-instruction or wall-clock, and
    /// `HostHooks` exposes no interrupt. So the caller is released on
    /// time and the engine is not stopped — see `sandbox` for why that
    /// trade beats freezing the client, and which runtimes do not need
    /// it.
    fn execute(&self, source: &str, ctx: &ExecContext<'_>) -> Result<ExecOutput, ExecError> {
        let owned = source.to_string();
        let root = ctx.workspace_root.clone();
        with_timeout(ctx.timeout, move || run_isolated(&owned, root))
    }
}

fn run_isolated(source: &str, workspace_root: std::path::PathBuf) -> Result<ExecOutput, ExecError> {
    let start = Instant::now();
    let mut context = Context::default();
    // Only `lang-query` reads the root; keep the parameter uniform.
    ...
    {
        let ws_root = Rc::new(workspace_root);
