...
pub fn move_block(
    ...
    if new_parent != current_parent {
        if let Some(old_page) = enclosing_page_id(&ctx.workspace, current_parent) {
            // A cross-page move dirties two pages, and `commit_page` is
            // scoped to one. The block's new page went through
            // `write_enclosing_page` above; this is the page it left.
            ctx.commit(old_page)?;
        }
    }
...
fn write_enclosing_page(ctx: &mut WsCtx, node: NodeId) -> Result<Option<PageMeta>, ApiError> {
    ...
    // The mutation already ran at the call site; this commits an empty
    // one for the projection and the steps around it.
    ctx.commit(page)?;
    Ok(meta)
}
