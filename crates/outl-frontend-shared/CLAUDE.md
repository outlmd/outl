...
└── src/
    ...
    │   ├── commands.ts     # invoke<T>() wrappers + re-exports of the three below
    │   ├── commands-peers.ts      # peer pairing / iroh transport
    │   ├── commands-plugins.ts    # plugin host + marketplace
    │   └── commands-reminders.ts  # `remind::` + its two display helpers
    ├── warnings/
    ...
    │   ├── mfu.test.ts
    │   ├── lock.ts          # the user's opt-out from MFU — port of Swift ToolbarLock
    │   ├── lock.test.ts
    │   ├── keys.test.ts     # pins both storage-key literals (cross-language contract)
    │   ├── storage.ts       # internal: defensive localStorage wrapper shared by mfu + lock
...
| `<EmbeddedSubtree nodes= embeds? depth? />` — read-only render of a `!((blk-X))` embed's source subtree as nested `↳` rows (TODO/DONE markers, nested refs/embeds via `embeds`), depth-capped at 4. Display-only, no onClick/edit/nav — the client wires the expansion under the carrying block | `@outl/shared/markdown` | mirror of `outl-tui`'s `emit_embedded_children` (`view/embed.rs`), `EMBED_MAX_DEPTH = 4` |
...
| `ParseWarning` / `ParseWarningKind` (DTO of `PageView.warnings`). `ParseWarningKind` has **six** members, in two families: `unrecognized_block_marker` (a line outside the outl dialect, kept verbatim) and five `remind_*` members (a `remind::` rule the scheduler could not take at face value: the block is fine, the *rule* is malformed). The five shipped with the reminder parser, mirrored here only after `outl-tauri-shared/tests/wire_enums.rs` gained the ability to fail on a missing enum variant; a `ParseWarning` is therefore **not** always "a line outside the dialect", and copy that says so is wrong for every `remind_*` kind | `@outl/shared/api/types` | `outl_md::ParseWarning` / `ParseWarningKind` |
...
| `PageBacklinks` (fields mirror the Rust struct) — reply of `pageBacklinks(slug)` / `setBacklinksOrder(...)`; backlinks are fetched lazily, decoupled from `PageView`, so the O(blocks) scan never blocks first paint | `@outl/shared/api/types` | `outl_tauri_shared::state::BacklinksReply` |
...
| Mobile keyboard-toolbar **logic** — `ToolbarAction` catalog + `TOOLBAR_META` (label + symbol/text style) + `DEFAULT_ORDER` + `PINNED_FIRST`/`PINNED_LAST` + `MIDDLE_ORDER` (the catalog minus the pinned slots — everything that reorders the bar permutes exactly this set, so both MFU and the lock read it instead of re-deriving the filter), plus the MFU ordering (`orderedMiddleActions(counts)` pure; `recordToStore`/`readCountsFromStore`/`orderedMiddleFromStore`/`clearCountsInStore` over `localStorage` under `outl.toolbar.mfu.v1`). The action string ids are the wire contract the iOS native bar ships via `window.__outlToolbar(action)`; **kept byte-identical to `OutlKit/Toolbar/ToolbarAction.swift`** (rename on both sides in one commit). Rendering (icons, capsule, keyboard docking) is client chrome, NOT here | `@outl/shared/toolbar` | port of `OutlKit/Toolbar/{ToolbarAction,ToolbarMFU}.swift` (the iOS native bar keeps its Swift copy because it renders before the webview) |
| Toolbar **order lock** — the user's opt-out from MFU reordering (#271). The barrel exports only what a client calls: `resolveMiddleFromStore()` (lay the row out), `lockOrderInStore` / `unlockOrderInStore` / `isOrderLocked`. The pure internals (`reconcileLockedOrder`, `parseLockedOrder`, `resolveMiddleOrder`, `LOCK_STORAGE_KEY`) stay module-level exports that only the tests import from `./lock`. **A lock stores an order, not a boolean**: counting keeps running while locked, so a flag alone would let the row jump on the next unlock or tiebreak change. `reconcileLockedOrder` is what keeps a snapshot honest as the catalog moves — unknown ids and the pinned slots dropped, actions the snapshot never saw appended at the end (appended, not inserted, because the user locked the row to stop things moving). Stored under `outl.toolbar.lock.v1` | `@outl/shared/toolbar` | `OutlKit/Toolbar/ToolbarLock.swift`. **Neither this nor the MFU counts is a per-bar store**: the settings sheet that writes them is web on iOS too, so `localStorage` is the single home and `OutlToolbar.swift` reads it (and the counts) out of the webview via `evaluateJavaScript`, through `OutlKit.ToolbarStore`. Both `LOCK_STORAGE_KEY` and `MFU_STORAGE_KEY` are pinned on both sides — `keys.test.ts` here, `ToolbarStoreTests` in Swift |
...
| `visibleRangeSlice(anchor, cursor, outline)` — the ids a selection range covers, DFS visible order. One owner for what every range op walks on both GUI clients, because the alternative was a comment in each client saying the copies matched. `null` when an endpoint is unset or no longer visible: `indexOf` returns `-1` and `ids.slice(-1, hi + 1)` is the **last block of the page**, not an empty range, so three of the four copies this replaced turned a stale anchor into a silent one-block op | `@outl/shared/outline` | one `flattenVisible` walk, where `visualRangeIds` plus a caller re-flattening cost two |
...
`<MarkdownInline />` and `<EmbeddedSubtree />` read only the canonical `--color-outl-*` custom properties (RFC 0022).
...
## `api/commands.ts` is split, and the import path is not

`commands.ts` grew past the repo's 600-line file ratchet, so peers,
plugins and reminders moved into `commands-peers.ts` /
`commands-plugins.ts` / `commands-reminders.ts`. `commands.ts`
re-exports all three, so **`@outl/shared/api/commands` stays the single
import path** and no call site changed. Add a new wrapper to whichever
file owns that area, and re-export it — a second import path is the
drift this library exists to prevent.
