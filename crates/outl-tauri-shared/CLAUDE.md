...
| `commands/open_with.rs` | `open_external_file(source_path)` — the OS "Open With → outl" import. A path + lock adapter only: normalise the string (`file://` URL or plain path, via `helpers::normalize_picker_path`), read the file **before** taking the workspace lock (the bytes are not workspace state), then run create + import as one `finish_in_page` commit. Every policy question — the `open-in/` namespace, the `source::` rule that makes re-opening navigate instead of duplicating, the extension / size / UTF-8 refusals — is `outl_actions::open_with`'s. Registered by **both** clients (`open_with_commands!`); only the desktop has an OS association delivering paths, which is `Capability::OpenExternalFile` = `Missing` on mobile |
