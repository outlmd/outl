//! Shortcut catalog command bodies.
//!
//! Moved out of `outl-desktop` so every GUI client registers the same
//! two commands through `outl_tauri_shared::shortcut_commands!`. Mobile
//! had neither, which meant the help overlay had no way to say *where*
//! an action exists — the exact question root `CLAUDE.md` invariant 12
//! exists to answer.
//!
//! Return the shared binding catalog so the
...
/// (a future feature).
pub fn list_shortcut_bindings() -> Vec<Binding> {
...
pub fn list_action_support() -> Vec<ActionSupportDto> {
