...
| `poll_jsonl_updates` (main loop, per tick) | Drains the signal. **In Insert mode**, sets `pending_reload = true` and returns — the in-flight `ParsedPage` would be clobbered by a reload mid-edit. Outside Insert, calls `engine.reload_workspace()` and `engine.reproject_page()` (via `reload_workspace_from_disk`). `reproject_page` is guarded (invariant 8) and can refuse when the focused page's `.md` holds content the merge never saw; the workspace still swaps in either way — a refusal is this one page's problem, not the reload's — and `reload_workspace_from_disk` toasts it (`ToastKind::Warning`) rather than swallowing it. |
