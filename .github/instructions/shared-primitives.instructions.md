...
| Build the `parent -> children` map once for a whole-tree walk (`children_of` is `O(nodes)` per call) | `outl_actions::build_children_index` | `crates/outl-actions/src/backlinks_index.rs` |
