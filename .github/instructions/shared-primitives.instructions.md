...
| Derive the `WorkspaceIndex` from the op-log tree (one-shot readers only — CLI, MCP; not a resident client's rebuild path, it materializes every block's text) | `outl_actions::index::derive` | `crates/outl-actions/src/index.rs` |
